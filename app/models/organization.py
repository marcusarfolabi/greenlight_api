...
class ArenaPayoutReport(Base):
    ...
    )  # Preserves identity if player account changes
    final_score: Mapped[int] = mapped_column(Integer, nullable=False)
    final_rank: Mapped[int] = mapped_column(Integer, nullable=False)

    # Stripe Integration States
    payout_amount_cents: Mapped[int] = mapped_column(
        Integer, default=0
    )  # Store in cents ($10.00 = 1000)
    ...
    payout_error_message: Mapped[str | None] = mapped_column(Text, nullable=True)
...
class PayoutRule(Base):
    ...
    arena_id: Mapped[str | None] = mapped_column(
        ForeignKey("arenas.id", ondelete="CASCADE"), nullable=True
    )

    position: Mapped[str] = mapped_column(
        String(50)
    )  # "1st", "2nd", "3rd", "top_5", etc.
