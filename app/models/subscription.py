...
class SubscriptionPlan(Base):
    ...
    currency: Mapped[str] = mapped_column(
        String(3), default="gbp"
    )  # "usd", "eur", "gbp", etc.
    interval: Mapped[str] = mapped_column(
        String(20), default="month"
    )  # "month" or "year"
    ...
    max_players: Mapped[int | None] = mapped_column(nullable=True)  # None = unlimited
    max_arenas: Mapped[int | None] = mapped_column(nullable=True)
    max_custom_themes: Mapped[int | None] = mapped_column(nullable=True)
    ...
    # AI Features
    ai_tokens: Mapped[int] = mapped_column(
        default=0
    )  # Monthly AI tokens for question generation, etc.
