...
async def create_arena(
    ...
    try:
        ...
        for q in data.questions:
            token_cost = (
                TokenService.calculate_question_cost(
                    len(q.prompt_text), len(q.options), True
                )
                if q.is_ai_generated
                else 0
            )
...
async def get_arena(
    ...
    completion_rate = (
        (completed_players / total_players * 100) if total_players > 0 else 0.0
    )
...
async def update_arena(
    ...
    if getattr(data, "questions", None) is not None:
        ...
        incoming_question_ids = (
            set()
        )  # Track which existing questions are in the update
        ...
        for q in qs:
            ...
            if q_id:
                existing_q = (
                    db.query(Question)
                    .filter(Question.id == q_id, Question.arena_id == arena_id)
                    .first()
                )
                ...
                if existing_q:
                    ...
                    existing_q.options_json = [
                        {"text": opt_text} for opt_text in q.options
                    ]

                    existing_q.type = getattr(q, "type", "multiple_choice")
                    existing_q.correct_option_index = q.correct_option_index
                    existing_q.correct_answer_string = getattr(
                        q, "correct_answer_string", None
                    )
            ...
            incoming_question_ids.add(
                new_q.id
            )  # <-- Add the fresh ID to the safe list so it won't be deleted!
        ...
        for q_to_delete in questions_to_delete:
            logger.info(
                f"Removing question {q_to_delete.id} to match frontend state sync"
            )
...
async def validate_access_code(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid access code format"
        )
    ...
    if not player_nickname or not player_nickname.strip():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="player_nickname is required",
        )
    ...
    try:
        ...
        if use_ai:
            try:
                ...
                parsed, errors = ai_parse_text_to_questions(
                    text, settings.GEMINI_API_KEY
                )
            except Exception as e:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"AI parse failed: {e}",
                )
        else:
    ...
    try:
        ...
        for q in parsed:
            if getattr(q, "is_ai_generated", False):
                total_ai_tokens += TokenService.calculate_question_cost(
                    len(q.prompt_text), len(q.options), use_ai_generation=True
                )
        ...
        if total_ai_tokens > 0:
            if not org:
                ...
                token_info["can_use"] = False
                token_info["message"] = (
                    "AI parsing is not enabled for your organization"
                )
            else:
        ...
        token_info = {
            "token_estimate": 0,
            "can_use": False,
            "message": "Failed to calculate token estimate",
        }
...
async def upload_questions(
    ...
    if arena.creator_id != current_user.user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Not authorized to upload questions for this arena",
        )
    ...
    try:
        ...
        if len(content) > MAX_UPLOAD_SIZE_BYTES:
            raise HTTPException(
                ...
                detail=f"File too large ({len(content)} bytes). Max allowed size is {MAX_UPLOAD_SIZE_BYTES} bytes",
            )
        ...
        if safe_filename.lower().endswith(".pdf"):
            try:
                ...
                if num_pages > MAX_PDF_PAGES:
                    raise HTTPException(
                        status_code=status.HTTP_400_BAD_REQUEST,
                        detail=f"PDF has {num_pages} pages which exceeds the maximum allowed {MAX_PDF_PAGES} pages",
                    )
            except HTTPException:
                raise
            except Exception:
        ...
        try:
            ...
            if total_ai_tokens > 0:
                if not org:
                    ...
                    can_use, msg = TokenService.can_use_tokens(
                        db, org.id, total_ai_tokens
                    )
                    token_info["can_use"] = can_use
                    token_info["message"] = msg
        except Exception:
...
async def send_participants_message(
    ...
    if arena.creator_organization_id:
        org = (
            db.query(User.owned_organization.property.mapper.class_)
            .filter_by(id=arena.creator_organization_id)
            .first()
        )
        if org:
            org_name = org.name
    ...
    if channel not in ["sms", "email"]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid channel"
        )
    ...
    if contacts:
        raw_contacts = [
            c.strip() for c in contacts.replace(",", "\n").split("\n") if c.strip()
        ]
    ...
    if not contacts_list:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="No valid contacts provided"
        )
    ...
    for contact in contacts_list:
        ...
        if channel == "sms":
            ...
            background_tasks.add_task(
                ...
                target,
                recipient_name,
                "You're invited to join an arena!",
                payload_message,
                {"arena_name": arena.arena_name, "access_code": arena.access_code},
                org_name,
            )
            queued += 1
    ...
    return {
        ...
        "queued": queued,
        "message": "messages queued for background delivery",
    }
...
async def start_arena_countdown(
    ...
    started = ws_manager.start_countdown(
        access_code, countdown_seconds, _countdown_broadcast
    )
